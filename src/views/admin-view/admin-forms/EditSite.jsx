...
import { createSite, getSiteById, updateSite } from "../../../api/sites";
...
const EditSite = () => {
  ...
  const { data, isSuccess: siteData } = getSiteById(id);
  const { mutate, isSuccess } = updateSite();
  const initialValues = {
    Address: "",
    Manager: "",
    phone: "",
  ...
  useEffect(() => {
    if (siteData && data) {
      ...
      formik.setFieldValue("Manager", data.data.data.Manager._id      );
      formik.setFieldValue("phone", data.data.data.phone);
      formik.setFieldValue("Allocated_budget", data.data.data.Allocated_budget);
      formik.setFieldValue(
        "Remaining_Budget",
        data.data.data.Remaining_Budget
      );
    }
  }, [siteData, data]);
  ...
  return (
    <form onSubmit={handleSubmit}>
      <span className="text-2xl font-bold">Edit Site</span>
